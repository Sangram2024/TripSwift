...
const getDestinationType = catchAsync (
  async (req:Request, res:Response, next:NextFunction) =>{
    const uniqueDestinationTypes = await PropertyAminite.distinct('destination_type');

    if (!uniqueDestinationTypes) {
      return next(
        new AppError(`No destination type found  `, 404)
      );
    }
...
export { createPropertyAminite, getPropertyAminiteById,getPropertyAminites, getDestinationType };
